...
function publicShare(){
	var modal = $('#modal1')
	modal.find('.modal-title').text('发布共享信息');
	var html = [];
	html.push('<div class="alert alert-warning px-3 text-center small">');
	html.push('<i class="fas fa-exclamation-triangle"></i>发布信息请遵守当地法律法规，文明共享！');
	html.push('</div>');
...
function initShareDisplay(data,containerClass){
	...
	for(var i=displaySize;i<loop;i++){
		...
		html.push('<span class="reply" data-toggle="modal" data-target="#modal1" data-sid='+data[i].id+' title="回复数"><i class="far fa-comment-alt"></i>&nbsp;&nbsp;'+data[i].comments.length+'</span>');
		html.push('<span class="reward" data-toggle="modal" data-target="#modal1" data-sid='+data[i].id+' title="累计打赏收入"><i class="far fa-credit-card"></i>&nbsp;&nbsp;'+calcReward(data[i].profits)+'&nbsp;NAS</span>');
		html.push('</span></div>');
	}
...
function showShareInfoInModal(sid){
	...
	html.push('</div>');
	html.push('<div class="col col-sm-2 px-3 text-center">');
	html.push('<button type="button" class="btn btn-danger rewardBtn" sid='+sid+' style="width:80px;">赏</button><p class="text-secondary small mt-1">共打赏'+share.profits.length+'次</p>');
	html.push('</div>');
	html.push('</div></div>');
	//reply
...
function quickReplyInModal(sid){
	...
	var html = [];
	html.push('<p class="text-lg-center">回复资源标题：'+share.title+'</p>');
	...
	html.push('<div class="invalid-feedback"> 请输入回复人昵称. </div>');
	html.push('</div>');
	html.push('<div class="form-group">');
...
function quickRewardInModal(sid){
	var share = getShareInfo(sid);
	var modal = $('#modal1');
	modal.find('.modal-title').text('打赏');
	var html = [];
	html.push('<p class="text-lg-center">打赏资源标题：'+share.title+'</p>');
	html.push('<div class="row justify-content-md-center pt-2">');
	...
	html.push('<div class="row justify-content-md-center pt-2">');
	html.push('<div class="col col-sm-2 px-3 text-center">');
	html.push('<button type="button" class="btn btn-danger" onclick="sendReward(\''+sid+'\');">确认打赏</button>');
	html.push('</div></div>');
	modal.find('.modal-body').html('');
	modal.find('.modal-body').html(html.join(''));
...
function sendReward(sid){
	if($('.rewardValueSelectedBtn').length == 0){
		...
		defaultOptions.listener = function(data){
			if(typeof data === "object"){
				tips('打赏成功，大约15秒后数据打包写入区块链，请稍后刷新查看。',true);
			}else{
				tips(data,false);
			}
		};
