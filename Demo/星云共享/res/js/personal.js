	...
	neb.api.getAccountState(addr).then(function (resp) {
		...
		neb.api.call({
			...
			if(resp.execute_err==""){
				...
				tips('您还没有在平台发布任何共享资料，个人中心无数据，请点击发布共享，赚取赏金！',false);
			}
		}).catch(function (err) {
			console.log(err);
		});
...
function initShareDisplay(data,containerClass){
	...
	for(var i=displaySize;i<loop;i++){
		...
		var statusLabel = data[i].status=='enable'?'停止分享':'未分享';
...
function resetShareStatus(sid){
	var share = getShareInfo(sid);
	var modal = $('#modal1')
	modal.find('.modal-title').text('资源分享状态设置');
	var html = [];
	html.push('<p class="text-lg-center">'+share.title+'</p>');
	html.push('<p class="text-center text-gray-dark small text-muted">作者：'+ share.ownner +" 发布时间："+ new Date(share.createtime*1000).toLocaleString()+'</p>');
	html.push('<span class="text-md-left text-muted">'+share.shareBody+'</span>');
	if(share.status == 'enable'){
		html.push('<div class="row mt-3 justify-content-md-center"><button type="button" class="btn btn-danger m-1" onclick=resetShareStatusAction("'+sid+'","disable");>停止分享</button></div>');
	}else{
		html.push('<div class="row mt-3 justify-content-md-center"><button type="button" class="btn btn-secondary m-1" onclick=resetShareStatusAction("'+sid+'","enable");>启用分享</button></div>');
	}
	modal.find('.modal-body').html('');
	modal.find('.modal-body').html(html.join(''));
}
function resetShareStatusAction(sid,status){
	...
	defaultOptions.listener = function(data){
		if(typeof data === "object"){
			tips('资源分享状态调整操作发送成功，大约15秒后数据打包写入区块链后完成状态修改，请稍后刷新查看。',true);
		}else{
			tips(data,false);
		}
	};
...
function editShare(sid){
	var share = getShareInfo(sid);
	var modal = $('#modal1')
	modal.find('.modal-title').text('修改共享内容');
	var html = [];
	html.push('<div class="alert alert-warning px-3 text-center small">');
	html.push('<i class="fas fa-exclamation-triangle"></i>发布信息请遵守当地法律法规，文明共享！');
	html.push('</div>');
...
function editShareAction(){
	...
	var validation = Array.prototype.filter.call(forms, function(form) {
		if (form.checkValidity() === false) {
			...
			defaultOptions.listener = function(data){
				if(typeof data === "object"){
					tips('修改共享成功，大约15秒后数据打包写入区块链，请稍后刷新查看。',true);
				}else{
					tips(data,false);
				}
			};
